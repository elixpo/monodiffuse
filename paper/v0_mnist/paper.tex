...
\IEEEauthorblockA{pollinations A\"I\\ayushman@pollinations.ai}
\and
...
central obstacle to running generative diffusion models on edge hardware, and a
growing contributor to the energy budget of deploying them at scale. Binarizing
weights to a single bit---replacing multiply--accumulate operations with
XNOR-popcount logic---would cut both memory and energy substantially, directly
serving the goals of sustainable, on-device AI. A widely
...
Post-Training Quantization, ResUNet, Edge AI, Sustainable Computing, Green AI, Energy-Efficient Deep Learning, MNIST
...
steps a typical DDPM requires is difficult on an edge processor today. It is also
an energy concern: the same floating-point arithmetic that limits deployability
drives the power draw and carbon cost of running these models, making
compute-efficient generation a question of sustainable computing as much as of
...
which keeps gradient flow through binary layers well-conditioned during
training (\emph{topological stability}). We present these as the two design
principles behind the method and develop each in the sections that follow.
...
(FID~29.60, 90.79\%). We do not read the small gap as evidence that the binary
model is genuinely \emph{better} than full precision; the fair conclusion is that
native binary-weight training incurs no meaningful loss on this benchmark. A fully
binary W1A1 variant (binary weights and activations) degrades to FID~83.98 but
remains far from collapse.

\textbf{Scope and contributions.} This is a small-scale study on a single, simple
dataset: MNIST digits occupy a far lower-dimensional manifold than natural images,
and the results here need not transfer to CIFAR-10, CelebA, or LSUN. Within that
scope, our contributions are:
\begin{itemize}
...
magnitude ordering the network relied on is discarded. We hypothesize that, over
the $T=1{,}000$ reverse steps, the resulting incoherence compounds and degrades
the sampled distribution---which is what we observe as the sharp FID increase
under PTQ in Section~VI.
...
(\emph{topological stability}). The following sections make each condition concrete
and test it empirically on MNIST.
...
legibility~90.79\%). We do not interpret the small FID difference as the binary
model being \emph{superior}: given the single dataset, the imperfect FID proxy
noted above, and run-to-run sampling variance, the appropriate reading is that
native binary-weight training \emph{matches} full precision on this benchmark
rather than beating it. PTQ, by contrast, degrades sharply: FID~191.94 (roughly
$7.9\times$ the native
...
\begin{figure}[t]
  \centering
  \subfloat{\includegraphics[width=0.35\columnwidth]{../../experiments/v0_mnist/assets/fp16_1.png}}\hfill
  \subfloat{\includegraphics[width=0.35\columnwidth]{../../experiments/v0_mnist/assets/fp16_2.png}}
...
\begin{figure}[t]
  \centering
  \subfloat{\includegraphics[width=0.35\columnwidth]{../../experiments/v0_mnist/assets/w1a16_our_output_1.png}}\hfill
  \subfloat{\includegraphics[width=0.35\columnwidth]{../../experiments/v0_mnist/assets/w1a16_our_output_2.png}}
...
\begin{figure}[t]
  \centering
  \subfloat{\includegraphics[width=0.35\columnwidth]{../../experiments/v0_mnist/assets/w1a1_our_output_1.png}}\hfill
  \subfloat{\includegraphics[width=0.35\columnwidth]{../../experiments/v0_mnist/assets/w1a1_our_output_2.png}}
...
\begin{figure}[t]
  \centering
  \subfloat[FP16]{\includegraphics[width=0.22\columnwidth]{../../experiments/v0_mnist/assets/fp16_1.png}}\hfill
  \subfloat[W1A16 Native]{\includegraphics[width=0.22\columnwidth]{../../experiments/v0_mnist/assets/w1a16_our_output_1.png}}\hfill
  \subfloat[W1A1 Native]{\includegraphics[width=0.22\columnwidth]{../../experiments/v0_mnist/assets/w1a1_our_output_1.png}}
...
baseline. From a sustainability standpoint, an order-of-magnitude reduction in
per-operation energy---if realised with dedicated binary kernels---would lower the
recurring inference cost of diffusion sampling, which is where deployed generative
models spend most of their energy over a model's lifetime. We view this as the
practical motivation for pursuing binary diffusion on edge and green-computing
platforms.
...
\smallskip\noindent\textbf{Reproducibility.} All results are reproducible from the
released repository. The three architectures (\texttt{ResUNet\_FP16},
\texttt{ResUNet\_W1A16}, \texttt{ResUNet\_W1A1}) and the binary primitives
(\texttt{BitConv2d\_Std}, \texttt{BitConv2d\_BNN}, and the clipped-STE binary
activation) are defined in a single \texttt{architectures} module. Training
scripts for the FP16/W1A16 and the W1A1 variants, the post-training quantizers
that produce the PTQ baselines, and the FID and Legibility benchmarks are provided
as separate entry points, and a single driver script reproduces the full results
table end-to-end. Trained checkpoints are released alongside the code so the
benchmarks can be run without retraining.
